...
use llvm_sys::core::{LLVMGetInstructionOpcode, LLVMIsTailCall, LLVMGetPreviousInstruction, LLVMGetNextInstruction, LLVMGetInstructionParent, LLVMInstructionEraseFromParent, LLVMInstructionClone, LLVMSetVolatile, LLVMGetVolatile, LLVMGetNumOperands, LLVMGetOperand, LLVMGetOperandUse, LLVMSetOperand, LLVMValueAsBasicBlock, LLVMIsABasicBlock, LLVMGetICmpPredicate};
#[llvm_versions(3.9 => latest)]
use llvm_sys::core::LLVMInstructionRemoveFromParent;
use llvm_sys::{LLVMOpcode, LLVMIntPredicate};
...
impl InstructionValue {
    ...
    /// Returns the predicate of an icmp instruction if any.
    pub fn get_icmp_predicate(&self) -> Option<IntPredicate> {
        let pred = unsafe {
            LLVMGetICmpPredicate(self.as_value_ref()) as isize
        };

        match pred {
            32 => Some(IntPredicate::EQ),
            33 => Some(IntPredicate::NE),
            34 => Some(IntPredicate::UGT),
            35 => Some(IntPredicate::UGE),
            36 => Some(IntPredicate::ULT),
            37 => Some(IntPredicate::ULE),
            38 => Some(IntPredicate::SGT),
            39 => Some(IntPredicate::SGE),
            40 => Some(IntPredicate::SLT),
            41 => Some(IntPredicate::SLE),
            _ => None,
        }
    }
}
